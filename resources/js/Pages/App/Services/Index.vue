...
defineProps({ services: [Object, Array], service: Object, totals: Object, services: Array, stock: [String, Number], averageCost: [String, Number], movements: Object });
</script>
<template>
    <AppLayout>
        ...
        <div class="space-y-3">
            <Card v-if="service">
                <h2 class="text-xl font-semibold">{ service.name || service.title || service.supplier_name || 'Услуги' }</h2>
                <p class="mt-2 text-sm text-slate-500">Карточка записи. Расширенное редактирование доступно в административной панели.</p>
            </Card>
            <Card v-for="row in (services.data || services || [])" :key="row.id">
