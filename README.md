...
- Filament для web/admin панели `/admin`
- Inertia + Vue 3 + Tailwind CSS для пользовательского приложения `/app`
...
Основной пользовательский интерфейс открывается по адресу `/app`. Если пользователь не вошёл, Laravel перенаправит на `/login`; маршрут `login` реализован собственной Inertia-страницей. Filament-панель открывается по адресу `/admin`.
...
- Публичная Inertia/Vue часть `/app`: dashboard, быстрые действия, формы покупок/списаний/услуг, склад, операции и аналитика.


## Публичная часть `/app`

Публичная часть — основной рабочий интерфейс владельца стройки после авторизации. Она отделена от Filament и использует те же backend Actions/Services, что API:

- `/app` и `/app/dashboard` — сводка расходов, склада и последние операции;
- `/app/operations` — единая лента покупок, списаний, услуг и корректировок;
- `/app/purchases/create` — быстрая покупка материалов с несколькими строками;
- `/app/write-offs/create` — быстрое списание с показом текущего остатка;
- `/app/services/create` — добавление услуги с типами fixed/hourly/unit;
- `/app/inventory` — мобильные карточки складских остатков;
- `/app/analytics` — фактические платежи и себестоимость этапов без задвоения расходов.

Vue-компоненты находятся в `resources/js/Components/App`, общий layout — `resources/js/Layouts/AppLayout.vue`, страницы — `resources/js/Pages/App`.
